...
COMPARE_BN_RELU_SRCS  = semi_compare_bn_relu.cc input_conv_cuda.cu torso_conv_cuda.cu output_conv_cuda.cu
...
COMPARE_BN_RELU_OBJS  = semi_compare_bn_relu.o input_conv_cuda.o torso_conv_cuda.o output_conv_cuda.o


# Target executables
COMPARE_TARGET    = compare_program
COMPARE_MODEL_TARGET    = compare_model_program
COMPARE_BN_RELU_TARGET = compare_bn_relu_program
...
# Build the semi CNN compare with BatchNorm and ReLU
$(COMPARE_BN_RELU_TARGET): $(COMPARE_BN_RELU_OBJS)
	$(CXX) $(LIBTORCH_CXXFLAGS) -o $@ $^ $(LIBTORCH_LDFLAGS)
...
# Compile rule for the BatchNorm and ReLU compare source file
semi_compare_bn_relu.o: semi_compare_bn_relu.cc
	$(CXX) $(LIBTORCH_CXXFLAGS) -c $< -o $@
...
# Run the comparing program with BatchNorm and ReLU
run_semi_compare_bn_relu: $(COMPARE_BN_RELU_TARGET)
	./$(COMPARE_BN_RELU_TARGET)

compare_cnn: run_semi_compare_cnn

compare_model: run_semi_compare_model

compare_bn_relu: run_semi_compare_bn_relu

clean:
	rm -f $(COMPARE_OBJS) $(COMPARE_MODEL_OBJS) $(COMPARE_TARGET) $(COMPARE_MODEL_TARGET) $(COMPARE_BN_RELU_TARGET) *.txt
