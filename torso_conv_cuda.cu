...
__global__ void torsoConvKernelV2(const float* __restrict__ d_input,
    ...
    __shared__ float sharedWeight[TORUS_INPUT_CHANNELS*TORUS_FILTER_SIZE*TORUS_FILTER_SIZE];
    for (int flatIndex = tid; flatIndex < TORUS_INPUT_CHANNELS*TORUS_FILTER_SIZE*TORUS_FILTER_SIZE; flatIndex += numT) {
        sharedWeight[flatIndex] = d_filter[TBid*TORUS_INPUT_CHANNELS*TORUS_FILTER_SIZE*TORUS_FILTER_SIZE + flatIndex];
    }
    // Synchronize after data copy is completed
    __syncthreads();
    ...
    for (int out_row = 0; out_row < out_height; out_row++) {
        ...
        for (int fr = 0; fr < filter_size; fr++) {
            for (int fc = 0; fc < filter_size; fc++) {
                ...
                //float weight = d_filter[(out_channel_idx * in_channels + in_channel_idx) * (filter_size * filter_size) + fr * filter_size + fc];
                int w_idx = in_channel_idx * (filter_size*filter_size)
                          + fr * filter_size + fc;
                float weight = sharedWeight[w_idx];
                sum += in_val * weight;
            }
        }
    // Atomically add the computed sum into the output pixel.
...
torch::Tensor torso_conv_forward(torch::Tensor input, torch::Tensor filter) {
    ...
    torsoConvKernelV2<<<out_channels, BLOCK_DIM_X * BLOCK_DIM_Y>>>(
